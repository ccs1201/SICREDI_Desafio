...
##### PS. Um cooperado só poderá votar se estiver cadastrado, por isso insira alguns *CPFs* fake para poder testar o endpoint de votação. Ou Simplesmente remova o "-" (underscore) do arquivo *afterMigrate.sql* que encontra-se em: *src/main/resources/db/migration/testdata* não esquece de comental-lô novamente após o primeiro startup, pois o FlyWay carrega este arquivo todo vez que a aplicação foir iniciada, como a coluna CPF tem uma restrição *UNIQUE* uma nova tentativa de inserção pelo FlyWay causá uma exception impedindo a aplicação de startar.
